...
var extendedJSON = (function() {
        ...
        [
            ...
            function(obj) {
                return JSON.stringify({
                    type: obj.constructor.name || "Error",
                    message: obj.message,
                    stack: obj.stack
                });
            },
                ...
                try {
                    ...
                    var s =
                        o instanceof Object && o
                            ? Object.getOwnPropertyDescriptor(o, out.pathSymbol)
                            : null;
    ...
    out.decode = function(obj, container, root) {
        if (!root) {
            var container = {};
            return out.decode(
                { type: "object", properties: { 0: ["root", obj] } },
                container,
                container
            ).root;
        }
        ...
        $.fn.jsConsole = function(data) {
            ...
            console.onRightClick(function(obj) {
                console.worker.postMessage({
                    type: "js-console-getData",
                    id: obj.outputLineData.workerDataID,
                    path: obj.getPath()
                });
            ...
            console.$removeWorkerReturnData = function(id) {
                //remove data of an object returned by the worker
                this.worker.postMessage({
                    type: "js-console-removeData",
                    id: id
                });
            };
                ...
                this.worker.onmessage = function(e) {
                    ...
                    if (data) {
                        if (data.type == "js-console-output") {
                            ...
                            if (data.trace) {
                                var pos = data.cmd.match(/timeEnd|time/)
                                    ? "push"
                                    : "unshift";
                                args[pos](
                                    new LineNumber(
                            ...
                            if (elData) {
                                ...
                                //no message was added, counter was incremented
            ...
            console.setForwardOutput = function(val) {
                ...
                console.$print = function(type) {
                    if (
                        window.console[type] &&
                        (val == true || val.indexOf(type) != -1) &&
                        window.console != console
                    ) {
                        var args = [];
                        for (var i = 1; i < arguments.length; i++) {
                            if (arguments[i] instanceof $.fn.console.LineNumber)
                                continue;
                            if (arguments[i] instanceof $.fn.console.PlainText)
                                args.push(arguments[i].text);
            ...
            if (data.forwardOutput)
                console.setForwardOutput(data.forwardOutput);
